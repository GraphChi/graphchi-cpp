...
void output_als_result(std::string filename) {
  MMOutputter<vertex_data> mmoutput_left(filename + "_U.mm", 0, M, "This file contains ALS output matrix U. In each row D factors of a single user node.", latent_factors_inmem);
  MMOutputter<vertex_data>  mmoutput_right(filename + "_V.mm", M, M+N, "This file contains ALS  output matrix V. In each row D factors of a single item node.", latent_factors_inmem);
